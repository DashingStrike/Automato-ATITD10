...
function doit()
  ...
  promptOkay("This macro is deprecated, and will be removed after blacksmithing.lua has been out for a while, so please try it and report any issues", 0xFF0000ff, nil, nil, true);
...
function findAnvil()
    ...
    sleepWithStatus(2000,"Pausing to compensate for server lag")
    ...
    srReadScreen();
    local pos = srFindImage("carpblade/scrap.png");
    if(not pos) then
        fatalError("Unable to find the scrap confirmation dialog.");
    end
    pos[0] = pos[0] + 10;
    local pos2 = waitForImage("yes.png", 500);
    if(not pos2) then
...
function takeBlade()
    ...
    waitForText("Complete Project", 500)
    clickText(pos);
    lsSleep(50);

    srReadScreen();
    pos = srFindImage("carpblade/unload.png");
    if(not pos) then
