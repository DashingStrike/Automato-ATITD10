...
function checkProcessing(ingredient)
  while 1 do
    ...
    process = findAllImages("processComplete.png")
      if #process == 0 then
        sleepWithStatus(999, "Processing ingrediant: " .. ingredient, nil, 0.7, "Monitoring Pinned Window(s)");
      else
        break; --We break this while statement because Processing Complete is not detect.
      end
  end
end
...
function getUserParams()
  ...
  config.board_index = 1;
    ...
    "Make sure you are carrying the EXACT number of boards you wish to treat.\n"
    .. "-----------------------------------------------------------");

    lsSetCamera(0,0,lsScreenX*1.4,lsScreenY*1.4);
    config.board_index = readSetting("board_name",config.board_index);
    lsPrint(10, current_y+70, 0, 0.95, 0.95, 0xffff40ff, "Treatment Recipe:");
    ...
    current_y = 105;
    lsPrint(10, current_y+40, 0, 0.95, 0.95, 0xffff40ff, "Volume of boards to treat:");
    ...
    got_user_params = true;
      ...
      current_y = current_y + 15;
        for i=1, #recipes[config.board_index].ingredient do
          drawWrappedText(math.ceil(recipes[config.board_index].amount[i] / 10) .. " "
          .. recipes[config.board_index].ingredient[i], 0xD0D0D0ff, X_PADDING, current_y+10);
          drawWrappedText(" : ", 0xFF0000ff, X_PADDING + 85, current_y+10);
          drawWrappedText(math.ceil(recipes[config.board_index].amount[i]) .. " Seconds "
          .. "", 0xFFFF40FF, X_PADDING + 105, current_y+10);
          current_y = current_y + 15;
        end
      end

      if lsButtonText(8, lsScreenY - 30, 0, 100, 0x00ff00ff, "Process") then
        is_done = 1;
      end
