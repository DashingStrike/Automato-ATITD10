...
function makeMetal(config, metal_amount)
  if metal_amount >= 500 then
    remainder = metal_amount % 500;
    fiveHundreds = metal_amount - remainder;
    fiveHundred_batches = fiveHundreds / 500;
    makeMetalBatch(config, fiveHundred_batches);
      if remainder > 0 then
        makeMetalBatch(config, 1, remainder);
      end
  else
    makeMetalBatch(config, metal_amount, nil, metal_amount)
  end
end

function makeMetalBatch(config, num_batches, remainder, batchSize)
