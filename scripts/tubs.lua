...
function doit()
  askForWindow("Automated tub related material production\n\n"
  .. "Pinned Window Mode\n- Automatically Steep 'Canvas' or 'Wool Cloth' in Clay\n\n"
  .. "Hotkey Mode\n- Automatically gather 'Water' and produce 'Potash & Salt',"
  .. " 'Saltpeter', 'Rotten Flax' or 'Pulp' from any given water type");
  promptParameters();
  if pinnedMode then
...
function promptParameters()
  ...
  while not is_done do
    ...
    if pinnedMode and not hotkeyMode then
      ...
      lsPrint(10, y-25, z, scale, scale, 0xFFFFFFff, "Steep in Clay:");
      steepInClay = lsDropdown("steepInClay", 155, 18, 0, 150, steepInClay, steepNames);
      writeSetting("steepInClay",steepInClay);
      y = y + 32;

      passCount = readSetting("passCount",passCount);
      lsPrint(10, y-13, z, scale, scale, 0xffffffff, "Passes:");
      is_done, passCount = lsEditBox("passes", 100, y-10, z, 50, 30, scale, scale, 0x000000ff, passCount);
      if not tonumber(passCount) then
      is_done = false;
      lsPrint(160, y-5, z+10, 0.7, 0.7, 0xFF2020ff, "MUST BE A NUMBER");
      passCount = 1;
      end
      y = y + 32;
    ...
    if hotkeyMode and not pinnedMode then
      lsPrint(10, y-20, z, scale, scale, 0xFFFFFFff, "Hotkey:");
      dropdown_cur_value = lsDropdown("ArrangerDropDown", 100, 20, 0, 200, dropdown_cur_value, dropdown_values);
      y = y + 32;

      hotkeyTask = readSetting("hotkeyTask",hotkeyTask);
      lsPrint(10, y-16, z, scale, scale, 0xFFFFFFff, "Task:");
      hotkeyTask = lsDropdown("hotkeyTask", 100, 60, 0, 200, hotkeyTask, hotkeyTaskNames);
      ...
      if not tonumber(passCount) then
      is_done = false;
      lsPrint(160, y, z+10, 0.7, 0.7, 0xFF2020ff, "MUST BE A NUMBER");
      passCount = 1;
      end
      y = y + 32;
      writeSetting("passCount",passCount);
    ...
    elseif hotkeyMode then
      ...
      if gatherWater then
        gatherWater = CheckBox(10, y+30, z, gatherWaterColor, " Auto Gather Water",
...
function checkSteeping()
  while 1 do
    ...
    sleepWithStatus(999, "Waiting for " .. product .. " to finish steeping",
    nil, 0.7, "Monitoring Pinned Window(s)");
  end
end
...
function tubAction()
  ...
  msTimerTeppyDuckOffset = (duckTeppyOffset * evaporationTimer) * 160 -- Add extra time to compensate for duck time
